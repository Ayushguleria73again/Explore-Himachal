...
export function InteractiveMap() {
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto py-20 px-6" onMouseMove={handleMouseMove}>
      ...
      <div className="relative aspect-square w-full bg-emerald-50/30 rounded-[3rem] border border-emerald-100 p-8 md:p-12 overflow-hidden shadow-inner flex items-center justify-center">
        <svg 
          ...
          {Object.entries(svgPaths).map(([name, d]) => {
            ...
            return (
              <motion.path
                ...
                animate={{ 
                  fill: isHovered ? "#10b981" : "#f0fdf4",
                  stroke: isHovered ? "#064e3b" : "#10b981",
        ...
        <AnimatePresence>
          {activeDistrict && (
            ...
            >
              <div className="bg-white/90 backdrop-blur-xl p-6 rounded-3xl shadow-2xl border border-white/20 w-72">
                <div className="mb-4">
                  <h4 className="text-[10px] uppercase tracking-widest text-emerald-500 font-black mb-1">Explore District</h4>
                  <p className="text-2xl font-black text-gray-950 tracking-tighter">{activeDistrict.name}</p>
                </div>
